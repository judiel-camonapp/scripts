global.data.pollAnswer = [false, false, false];
...
global.initPoll = function() {
    experience.currentScene.shapesByNameLike("poll_dialog").forEach(el => el.clickable = false);
    experience.currentScene.shapesByNameLike("pre_poll").forEach(el => el.clickable = false);

    setTimeout(function() {
        experience.currentScene.shapesByNameLike("pre_poll").forEach(el => {
            global.fadeIn(el);
            el.clickable = true;
        });
    }, 60000);
...
global.showPoll = function() {
    global.restorePollAnswerButtons();

    experience.currentScene.shapesByNameLike("pre_poll").forEach(el => {
        global.fadeOut(el);
        el.clickable = false;
    });
    experience.currentScene.shapesByNameLike("logo").forEach(el => global.fadeOut(el));
    experience.currentScene.shapesByNameLike("poll_dialog").forEach(el => global.fadeIn(el));
    experience.currentScene.shapesByNameLike("poll_dialog_btn").forEach(el => el.clickable = true);
}

global.hidePoll = function(pollAnswered) {
    experience.currentScene.shapesByNameLike("poll_dialog").forEach(el => global.fadeOut(el));
    experience.currentScene.shapesByNameLike("poll_dialog_btn").forEach(el => el.clickable = false);
    if (!pollAnswered) {
        experience.currentScene.shapesByNameLike("pre_poll").forEach(el => {
            global.fadeIn(el);
            el.clickable = true;
        ...
        experience.currentScene.shapeByName("poll_thanks").fadeOut();
    }

    setTimeout(function() {
        experience.currentScene.shapesByNameLike("logo").forEach(el => global.fadeIn(el));
        experience.currentScene.shapesByNameLike("poll_dialog_tmp").forEach(el => el.remove());
    }, 1000);
}

global.pollThanks = function() {
    experience.currentScene.shapesByNameLike("poll_dialog").forEach(el => {
        global.fadeOut(el);
        el.clickable = false;
    });
    experience.currentScene.shapeByName("poll_thanks").fadeIn();
}

global.sendPoll = function() {
    global.pollThanks();
    setTimeout(function() { global.hidePoll(true); }, 5000);
}

global.restorePollAnswerButtons = function() {
    [1, 2, 3].forEach(i => {
        global.fadeIn(experience.currentScene.shapeByName("poll_dialog_btn_" + i + "_yes"));
        experience.currentScene.shapeByName("poll_dialog_btn_" + i + "_yes").clickable = true;
        global.fadeIn(experience.currentScene.shapeByName("poll_dialog_btn_" + i + "_no"));
        experience.currentScene.shapeByName("poll_dialog_btn_" + i + "_no").clickable = true;
    });
}

global.answerPollQuestion = function(question, answer) {
    global.data.pollAnswer[question - 1] = answer;

    var yes = experience.currentScene.shapeByName("poll_dialog_btn_" + question + "_yes");
    var no = experience.currentScene.shapeByName("poll_dialog_btn_" + question + "_no");
    ...
    experience.currentScene.newElement({
        type: "sprite",
        name: "poll_dialog_tmp_" + question,
        position: answer ? yes.position : no.position,
        scale: 0.35,
        asset: answer ? "Btn Si hover.png" : "Btn No hover.png"
    });
}